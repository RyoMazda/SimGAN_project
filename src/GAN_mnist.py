...
def train():
    ...
    D_W1 = weight_variable([X_dim, hidden_D])
    D_b1 = bias_variable([hidden_D], init_const=0)

    # D_W2 = tf.Variable(tf.zeros([hidden_D, Y_dim]))
    D_W2 = weight_variable([hidden_D, Y_dim])
    D_b2 = bias_variable([Y_dim], init_const=0)
    ...
    def discriminator(x):
        ...
        """
        D_h1 = tf.nn.bias_add(tf.matmul(x, D_W1), D_b1)
        # D_h1 = batch_normalize(D_h1)
    ...
    def generator(z):
        G_h1 = tf.nn.bias_add(tf.matmul(z, G_W1), G_b1)
        G_h1 = batch_normalize(G_h1, shift=shift_G1, scale=scale_G1)
        G_h1 = lrelu(G_h1)
    ...
    if is_separate:
        fm = myutil.FileManager('GAN_separate')
    else:
        fm = myutil.FileManager('GAN_mnist')
    fm.mkdir()
