...
def train_refiner(x_fake, x_real):
    ...
    # ---------------------------
    # first layer
    W_D1 = weight_variable([mask_size, mask_size, 1, D1_dim])
    b_D1 = bias_variable([D1_dim])
    # second layer
    W_D2 = weight_variable([mask_size, mask_size, D1_dim, D2_dim])
    b_D2 = bias_variable([D2_dim])
    # third layer
    W_D3 = weight_variable([(pix_size // 4) ** 2 * D2_dim, D3_dim])
    b_D3 = bias_variable([D3_dim])
    ...
    W_R0 = weight_variable([mask_size, mask_size, 1, R1_dim])
    W_R1 = weight_variable([mask_size, mask_size, R1_dim, R2_dim])
    W_R2 = weight_variable([mask_size, mask_size, R2_dim, R3_dim])
    b_R0 = bias_variable([R1_dim])
    b_R1 = bias_variable([R2_dim])
    b_R2 = bias_variable([R3_dim])

    # pack Discriminator variables
    theta_R = [W_R0, b_R0, W_R1, b_R1, W_R2, b_R2]

    def refiner(x):
        x_4d = tf.reshape(x, [-1, pix_size, pix_size, 1])

        h_R0 = tf.nn.bias_add(conv2d(x_4d, W_R0, stride=1), b_R0)
        h_R0 = lrelu(batch_normalize(h_R0))
        h_R1 = tf.nn.bias_add(conv2d(h_R0, W_R1, stride=1), b_R1)
        h_R1 = lrelu(batch_normalize(h_R1))
...
def main():
    ...
    # preprocess_fonts("font_images/824F")

    # load synthesized images(numbers out of fonts) with labels
    x_fake = load_fonts_as_np_array()
...
def preprocess_fonts(dir_path):
    index = 0
    for image_path in glob.glob(dir_path + "/*.png"):
        img = Image.open(image_path)
        img = img.convert('L')
        img = img.resize((28, 28))
        path = "font_images/zeros/" + str(index) + ".png"
        img.save(path)
...
def load_fonts_as_np_array():
    fake_images = []

    for path in glob.glob("font_images/zeros/*.png"):
        img = imread(path)
